...
namespace OneDriveApiBrowser
{
    ...
    public partial class FormBrowser : Form
    {
        ...
        private async void deleteSelectedItemToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                // Prompts the user to enter in the access key
                AuthenticationHelper.SignedOut = true;
        ...
        {
            FormInputDialog dialog = new FormInputDialog("Create File", "File name:");
            var result = dialog.ShowDialog();
            if (result == System.Windows.Forms.DialogResult.OK && !string.IsNullOrEmpty(dialog.InputText))
            {
                try
                {
                    var fileToCreate = new DriveItem { Name = dialog.InputText, File = new File() };
                    var newFile =
                        await this.graphClient.Drive.Items[this.SelectedItem.Id].Children.Request()
                            .AddAsync(fileToCreate);

                    if (newFile != null)
                    {
                        MessageBox.Show("Created new file with ID " + newFile.Id);
                        this.AddItemToFolderContents(newFile);
                    }
                }
                catch (ServiceException exception)
                {
